...
function dateParse(v: number) {
  return months[v % 12];
}
function mutateMonth(v: number, m: number, y: number) {
  let temp = m + v;
  return temp < 0 ? { YYYY: y - 1, MM: dateParse(12 + temp) } : { YYYY: y + Math.floor(temp / 12), MM: dateParse(temp) };
}
export function fromToFormat(dir: number, year: number, month: number): [string, string, number, number] {
  const Forward = dir > 0;
